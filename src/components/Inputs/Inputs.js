...
export default function Inputs() {
    ...
    const handleSubmitNickname = async (e) => {
        ...
        if (inputsNickname === '') {
            ...
            setShowSaveInputs(false);
            try {
                await authRequest.post('/api/choice-model/', 
                    {
    ...
    const getModels = async (history_id) => {
        const models = await authRequest.get(`/account/${history_id}/models/`);
        return models;
    }

    const getModelData = async (model_id) => {
        try {
            const inputData = await authRequest.get(`/api/choice-model/${model_id}/`);
    ...
    const deleteModelData = async (model_id) => {
        try {
            await authRequest.delete(`/api/choice-model/${model_id}/`);
            setErrorMessage(false);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await authRequest.post(`/api/calculate/`,
                {
                    history_id: historyId,
                    off_peak_a: offPeakA,
                    off_peak_b: offPeakB,
